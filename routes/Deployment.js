...
const createContainerOptions = (config, volumePath) => {
    // Get startup command from egg if available
    const startupCommand = config.StartupCommand || config.Cmd;
    const stopCommand = config.StopCommand;

    // Handle egg environment variables
    const eggEnv = config.eggData?.Variables?.reduce((acc, v) => {
        acc[v.name] = v.defaultValue;
        return acc;
    }, {}) || {};

    // Merge egg env with provided env
    const mergedEnv = [...(config.Env || []), ...objectToEnv(eggEnv)];
    ...
    return {
        ...
        Env: mergedEnv,
        ...(startupCommand && { Cmd: startupCommand }),
        ...(stopCommand && { StopSignal: stopCommand })
    };
};
...
const createContainer = async (req, res) => {
    ...
    let { 
        Image, Id, Cmd, Env, Ports, Scripts, Memory, Cpu, PortBindings,
        StartupCommand, StopCommand, eggData, DockerImages
    } = req.body;
    ...
    try {
        ...
        // Handle egg environment variables
        const eggEnv = eggData?.Variables?.reduce((acc, v) => {
            acc[v.name] = v.defaultValue;
            return acc;
        }, {}) || {};
        ...
        // Handle egg Docker images
        const finalImage = DockerImages?.[variables.version || 'latest'] || Image;

        log.info(`Pulling image: ${finalImage}`);
        try {
            const stream = await docker.pull(finalImage);
            await new Promise((resolve, reject) => {
                docker.modem.followProgress(stream, (err, result) => {
                    if (err) {
                        return reject(new Error(`Failed to pull image: ${err.message}`));
                    }
                    log.info(`Image ${finalImage} pulled successfully.`);
                    resolve(result);
                });
            });
        } catch (err) {
            log.error(`Error pulling image ${finalImage}:`, err);
